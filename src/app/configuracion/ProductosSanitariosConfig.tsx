...
import { crearProductoSanitario, actualizarProductoSanitario, toggleActivo } from "./actions";

type ProductoSanitario = { id: string; nombre: string; precio_por_unidad: number; unidad: string; activo: boolean };

export default function ProductosSanitariosConfig({ items }: { items: ProductoSanitario[] }) {
  const router = useRouter();
  const [mostrarNuevo, setMostrarNuevo] = useState(false);
  const [nuevoNombre, setNuevoNombre] = useState("");
  const [nuevoPrecio, setNuevoPrecio] = useState("");
  const [nuevaUnidad, setNuevaUnidad] = useState("ml");
  const [editandoId, setEditandoId] = useState<string | null>(null);
  const [editNombre, setEditNombre] = useState("");
  const [editPrecio, setEditPrecio] = useState("");
  const [editUnidad, setEditUnidad] = useState("ml");
  ...
  async function handleCrear() {
    ...
    setGuardando(true);
    setError(null);
    const res = await crearProductoSanitario({ nombre: nuevoNombre, precio_por_unidad: parseFloat(nuevoPrecio), unidad: nuevaUnidad });
    if (res.ok) {
      setNuevoNombre("");
      setNuevoPrecio("");
      setNuevaUnidad("ml");
  ...
  async function handleActualizar(id: string) {
    ...
    setGuardando(true);
    setError(null);
    const res = await actualizarProductoSanitario(id, { nombre: editNombre, precio_por_unidad: parseFloat(editPrecio), unidad: editUnidad });
    if (res.ok) {
      setEditandoId(null);
      router.refresh();
    } else {
  ...
  async function handleToggle(id: string, activo: boolean) {
    setGuardando(true);
    await toggleActivo("productos_sanitarios", id, !activo);
    router.refresh();
    setGuardando(false);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {mostrarNuevo ? (
        <div className="flex gap-2 items-center flex-wrap">
          ...
          <div className="flex items-center gap-1">
            ...
            <input
              ...
              placeholder="Precio"
              className="border border-stone-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-300 w-32"
            />
            ...
            <input
              ...
              placeholder="ml"
              className="border border-stone-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-300 w-20"
            />
          </div>
          <button
            onClick={handleCrear}
          ...
          <button
            onClick={() => { setMostrarNuevo(false); setNuevoNombre(""); setNuevoPrecio(""); setNuevaUnidad("ml"); }}
        ...
        >
          + Agregar producto sanitario
        </button>
      )}

      <div className="bg-white rounded-xl border border-stone-200 shadow-sm overflow-x-auto">
        {items.length === 0 ? (
          <p className="px-4 py-8 text-center text-sm text-stone-400">No hay productos sanitarios cargados aún</p>
        ) : (
                ...
                editandoId === item.id ? (
                    ...
                    <td className="px-4 py-2">
                      <div className="flex items-center gap-1 justify-end">
                        ...
                        <input
                          ...
                          className="w-28 border border-stone-300 rounded px-2 py-1 text-sm text-right focus:outline-none focus:ring-1 focus:ring-green-300"
                        />
                        <span className="text-stone-400 text-xs">/</span>
                        <input
                          type="text"
                          ...
                          className="w-16 border border-stone-300 rounded px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-green-300"
                        />
                      </div>
                    </td>
                  ...
                  <tr key={item.id} className={`hover:bg-stone-50 ${!item.activo ? "opacity-50" : ""}`}>
                    ...
                    <td className="px-4 py-3 text-stone-700 text-right font-mono">
                      ${Number(item.precio_por_unidad).toLocaleString("es-AR", { minimumFractionDigits: 2 })}
                      <span className="text-stone-400 text-xs font-sans ml-1">/{item.unidad || "ml"}</span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex gap-2 justify-end">
                        <button
                          onClick={() => { setEditandoId(item.id); setEditNombre(item.nombre); setEditPrecio(String(item.precio_por_unidad)); setEditUnidad(item.unidad || "ml"); setMostrarNuevo(false); }}
