    ...
    <div className="space-y-6">
      ...
      {(trabajos ?? []).length === 0 ? (
        <div className="bg-white rounded-xl border border-stone-200 p-10 text-center shadow-sm">
          <p className="text-stone-400 text-sm">No hay trabajos registrados todavía. Importá el CSV del bastón para comenzar.</p>
        </div>
      ) : (
        <>
          {/* Mobile cards */}
            ...
            {(trabajos ?? []).map((t) => (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="shrink-0 text-xs font-mono bg-stone-100 text-stone-600 px-2 py-1 rounded-full">
                    {t.total_chips} chips
                  </span>
                </div>
